...
oil_table <- data.frame(format(as.Date(oil$date,format="%y-%m-%d"),"%d/%m/%Y"),
                        paste(oil$olives_sp+oil$olives_f," Kg"),
                        paste(oil$production," Kg"),
                        paste(oil$oil_return*100," %"),
                        paste(oil$prod_litre," Lt"),
                        paste("EUR ",oil$tot_cost),
                        paste("EUR ",round(oil$tot_cost/oil$prod_litre,digit=2)))
...
###1###
oil_plot <- barplot(cbind(oil$olives_sp,oil$olives_f)~oil$date,
                    beside=TRUE,ylim=c(0,200),col=c("#00A600","#EAB64E"),
                    xaxt="n",xlab="",ylab="Kg",
                    main="Olive raccolte (Kg)")
axis(1,at=c(2,5),labels=oil$date)
text(x=t(oil_plot),y=cbind(oil$olives_sp,oil$olives_f),
     labels=cbind(paste(c(oil$olives_sp,oil$olives_f)," Kg")),
...
###2###
oil_plot2 <- barplot(cbind(sum(oil$olives_sp),sum(oil$olives_f)),
                     beside=TRUE,space=c(1,.4),ylim=c(0,200),col=c("#00A600","#EAB64E"),
                     main="Olive raccolte per campagna (Kg)")
axis(1,at=oil_plot2,labels=c("San Placido","Foddiri"))
text(x=oil_plot2,y=cbind(sum(oil$olives_sp),sum(oil$olives_f)),
     labels=paste(cbind(sum(oil$olives_sp),sum(oil$olives_f))," Kg"),pos=3,cex=.9)
